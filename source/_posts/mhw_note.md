...
本文的全篇内容都是在基于玩家拥有冰原DLC的情况下讨论的。

## 设置修改

游戏设置 -> 视角的导虫自动追踪设置：不追踪

操控 -> 自动纳刀设置：关

操控 -> 键盘设置 - > 近身武器设置 -> 武器·显示投射器的准心（按一次）：`V`
...
装备技能 **减轻胆怯（耐冲）**，其中一条描述为：操虫棍萃取效果部分效果上升。具体就是点满三灯的时候每一级耐冲会在你的技能基础上额外加一级耳塞、耐震、风压耐性，出满三级减轻胆怯的技能就能在点满三灯的时候提高你的站场能力。
...
飞圆斩只有在点亮红灯的条件下才能触发，下面是大部分能派生飞圆斩的连招：
...
左键突刺 + 左键袈裟斩 + 左键二连斩 + 右键飞圆斩

W左键突刺 + S右键回避斩 + 右键飞圆斩

空中侧键急袭突刺（撑杆跳下戳） + 右键飞圆斩
...
不建议在通关主线之前刷猫饭食材，一般你要用到的猫饭只有增加攻击力的肉套餐，以及包含能增加猫的次数的「猫的生命保险」的迎宾酒套餐，建议的是找个刷「猫的生命保险」的教程看一遍。全食材手收集攻略：
- [Bilibili - 【MHWI】猫饭全收集](https://www.bilibili.com/read/readlist/rl370733)