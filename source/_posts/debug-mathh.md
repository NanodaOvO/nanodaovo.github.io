...
金色的太阳高高的挂在碧蓝的天空上，猫咪懒洋洋的卧在太阳充足的地方，贪婪的享受着温暖，时不时还舔一舔自己的爪子。
...
但是当我把代码一字不差地输入之后，却没有生成可执行文件，工作区也没有检测到语法错误。无论用的是哪种计算方法，似乎只要 include 了 math.h ，编译便会诡异地中止，
...
虽然一眼看上去字很多，但其中大部分都是目标文件的名称和路径这些无用信息，提取其中的关键信息才是首要的；
...
开发环境：WSL＋GCC＋VScode，Linux版本是 Ubuntu 22.04.3
